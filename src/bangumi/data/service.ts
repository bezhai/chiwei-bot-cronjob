...
export class BangumiDataService {
  ...
  async findSubjects(
    ...
    try {
      const cursor = BangumiSubjectCollection.find(filter, options);
  ...
  ): Promise<BangumiCharacterDocument[]> {
    try {
      const cursor = BangumiCharacterCollection.find(filter, options);
      return await cursor.toArray();
    } catch (error) {
   ...
   * 批量删除条目
   */
  async deleteSubjects(ids: number[]): Promise<number> {
    try {
      const result = await BangumiSubjectCollection.deleteMany({ id: { $in: ids } });
   ...
   * 批量删除角色
   */
  async deleteCharacters(ids: number[]): Promise<number> {
    try {
      const result = await BangumiCharacterCollection.deleteMany({ id: { $in: ids } });
      return result.deletedCount || 0;
    } catch (error) {
