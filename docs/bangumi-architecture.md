...
使用 `bangumiManager.ts` 脚本管理同步任务：

```bash
# 执行同步
npm run bangumi sync:daily
npm run bangumi sync:yearly
npm run bangumi sync:biweekly
npm run bangumi sync:monthly

# 月份轮询管理
npm run bangumi rotation:status
npm run bangumi rotation:reset 0

# 策略管理
npm run bangumi strategy:list
npm run bangumi strategy:stop DailyIncremental
npm run bangumi strategy:stop-all

# 其他
npm run bangumi stats
npm run bangumi config:show
```
...
1. **分层清晰**：各层职责明确，便于维护
2. **高度可配置**：支持环境变量和运行时配置
3. **易于扩展**：策略模式支持快速添加新的同步策略
4. **统一管理**：集中的限流、错误处理和日志
5. **性能优化**：批量处理、生成器模式、智能冷却机制
6. **监控友好**：提供进度跟踪和统计信息
...
1. 删除旧的服务文件：
   - `src/service/bangumiService.ts`
   - `src/service/bangumiSyncService.ts`
   - `src/service/bangumiGradualSyncService.ts`
   - `src/service/bangumiMonthlyRotationService.ts`

2. 更新导入路径：
   ```typescript
   // 旧
   import { dailyIncrementalUpdate } from './service/bangumiGradualSyncService';
   
   // 新
   import { dailyIncrementalUpdate } from './bangumi';
   ```

3. 使用新的API：
   ```typescript
   // 使用策略管理器
   import { strategyManager } from './bangumi';
   const result = await strategyManager.executeStrategy('DailyIncremental');
   ```

## 后续优化建议

1. 添加更多同步策略（如按标签、按评分等）
2. 实现并发控制优化
3. 添加 WebSocket 支持实时进度推送
4. 增加数据校验和修复功能
5. 实现增量索引优化查询性能